...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onNavigate }) => {
  ...
  return (
    <div
      ...
      className="group bg-white rounded-2xl border border-slate-200/80 hover:border-emerald-300 hover:shadow-xl hover:shadow-emerald-950/5 transition-all duration-300 flex flex-col overflow-hidden cursor-pointer relative"
    >
      {/* Image Area */}
      <div className="relative aspect-4/3 sm:aspect-square bg-slate-50 overflow-hidden">
        ...
        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1 items-start z-10">
          {product.discountPercentage > 0 && (
            <span className="bg-rose-500 text-white text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider shadow-xs">
      ...
      <div className="p-3.5 sm:p-4 flex-1 flex flex-col justify-between">
        <div>
          ...
          {/* Title */}
          <h3 className="text-xs sm:text-sm font-bold text-slate-800 line-clamp-1 group-hover:text-emerald-700 transition-colors">
            {product.name}
          </h3>
          ...
          <div>
            {isOutOfStock ? (
              ...
              <div 
                ...
                className="flex items-center bg-emerald-700 text-white rounded-xl overflow-hidden shadow-xs"
              >
                <button
                  onClick={handleDecrement}
                  className="w-7 h-7 flex items-center justify-center hover:bg-emerald-800 transition-colors"
                >
              ...
              <button
                ...
                className="px-3 py-1.5 sm:px-3.5 sm:py-2 bg-emerald-50 hover:bg-emerald-600 text-emerald-700 hover:text-white border border-emerald-200 hover:border-emerald-600 rounded-xl text-xs font-bold transition-all flex items-center gap-1 active:scale-95 shadow-2xs"
              >
                <Plus className="w-3.5 h-3.5" />
                <span>Add</span>
              </button>
